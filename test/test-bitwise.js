...
contract("BitWise", () =>{
   ...
   before(() => {
    ...
    it('countBitSet() should pass', async () => {
        ...
        expect(gas).to.be.gt(gasAsm, "gas efficient has to be prent in Assembly");

        const result = await bitwise.countBitSet(myNumber);
        const resultAsm = await bitwise.countBitSetAsm(myNumber);
        expect(result).to.be.a.bignumber.that.equal(resultAsm, "result should be similar");
    })
    
    it('test case for countBitSetAsm', async () => {
        data = 0;
        const resultAsm = await bitwise.countBitSetAsm(0);
        assert(resultAsm, 0, "mstching result");
    })
}) 